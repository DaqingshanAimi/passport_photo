...
def resize_photo(photo, choice):
    ...
    if choice == 2:
        ...
        return resized_photo
def layout_photo_1(photo, color=(255, 255, 255)):
    bk = Image.new("RGB", [WIDTH_1IN, HEIGHT_1IN], color)
    bk.paste(photo, (0, 0))
    return bk
def layout_photo_2(photo, color=(255, 255, 255)):
    bk = Image.new("RGB", [WIDTH_2IN, HEIGHT_2IN], color)
    bk.paste(photo, (0, 0))
    return bk
...
def design_photo(image, size, color=(255, 255, 255)):
    if size == 's1x5':
        ...
        return layout_photo_1(resize_photo(cut_photo(image, 1), 1), color=color)
    elif size == 's2':
        return layout_photo_2(resize_photo(cut_photo(image, 2), 2), color=color)
    elif size == 's1x6':
